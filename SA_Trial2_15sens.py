...
os.makedirs('/beegfs/.global1/ws/arsi805e-Test/New/Reduced_Sensors_Models_SA/model_' + str(batch_size) + '_' + str(num_coupling_nodes) + '_' + str(num_subnet_layers +3) + '_' + str(n_epochs) + '_' + str(lr) + '_15sens', exist_ok=True)

logging.basicConfig(filename='/beegfs/.global1/ws/arsi805e-Test/New/Reduced_Sensors_Models_SA/model_' + str(batch_size) + '_' + str(num_coupling_nodes) + '_' + str(num_subnet_layers +3) + '_' + str(n_epochs) + '_' + str(lr) + '_15sens' + '/Info.log', level=logging.INFO)
...
def simulated_annealing(initial_state, objective_function, max_iterations, initial_temp, cooling_rate):
    ...
    for iteration in range(max_iterations):
        ...
        # Choose a sensor to deactivate and a sensor to activate
        active_sensors = np.where(next_state)[0]
        inactive_sensors = np.where(~next_state)[0]
        
        sensor_to_deactivate = np.random.choice(active_sensors)
        sensor_to_activate = np.random.choice(inactive_sensors)
        
        # Swap the active sensor with an inactive one
        next_state[sensor_to_deactivate] = False
        next_state[sensor_to_activate] = True
        
        # Evaluate the new configuration
        next_loss = objective_function(next_state)
        
        # Decide whether to accept the new configuration
        if next_loss < best_loss or np.random.rand() < np.exp(-(next_loss - best_loss) / current_temp):
        ...
        dropped_sens_current = [index+1 for index, value in enumerate(next_state) if not value]
        print(f"Iteration {iteration}: Sensor Config: {best_state}, \nBest Loss: {best_loss}")
        print(f"Dropped Sensors: {dropped_sens_current}")
        logging.info(f"Iteration {iteration}: \nBest Sensor Config: {best_state}, \nBest Loss: {best_loss} \nCurrent Sensor Config:{next_state} \nCurrent Loss:{next_loss}")
        logging.info(f"Dropped Sensors for Curr. Loss: {dropped_sens_current}")
        logging.info(f'Active Sensors for Curr. Loss: {100-len(dropped_sens_current)}')
        logging.info(f'Sensor Deactivated: {sensor_to_deactivate}')
        logging.info(f'Sensor Activated: {sensor_to_activate}')
...
def evaluate_model_with_sensors(sensors_config):
    ...
    patience = 10
    if EARLY_STOP:    
        early_stopping = EarlyStopping(patience=patience, verbose=True, path='/beegfs/.global1/ws/arsi805e-Test/New/Reduced_Sensors_Models_SA/model_' + str(batch_size) + '_' + str(num_coupling_nodes) + '_' + str(num_subnet_layers +3) + '_' + str(n_epochs) + '_' + str(lr) + '_15sens' + '/model_checkpoint.pth')
    ...
    torch.save(state, '/beegfs/.global1/ws/arsi805e-Test/New/Reduced_Sensors_Models_SA/model_' + str(batch_size) + '_' + str(num_coupling_nodes) + '_' + str(num_subnet_layers +3) + '_' + str(n_epochs) + '_' + str(lr) + '_15sens' + '/model_'  + str(batch_size) + '_'  + str(num_coupling_nodes) + '_' + str(num_subnet_layers+3) + '_' + str(n_epochs) + '_' + str(lr) + '_15sens' + '.pth')
...
# Initialize with 15 active sensors
initial_sensor_state = np.zeros(100, dtype=bool)
initial_sensor_state[:15] = True
np.random.shuffle(initial_sensor_state)
