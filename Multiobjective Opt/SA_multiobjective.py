...
os.makedirs('/beegfs/.global1/ws/arsi805e-Test/New/Reduced_Sensors_Models_SA/model_' + str(batch_size) + '_' + str(num_coupling_nodes) + '_' + str(num_subnet_layers +3) + '_' + str(n_epochs) + '_' + str(lr) + '_multiobj', exist_ok=True)

logging.basicConfig(filename='/beegfs/.global1/ws/arsi805e-Test/New/Reduced_Sensors_Models_SA/model_' + str(batch_size) + '_' + str(num_coupling_nodes) + '_' + str(num_subnet_layers +3) + '_' + str(n_epochs) + '_' + str(lr) + '_multiobj' + '/Info.log', level=logging.INFO)
...
def evaluate_model_with_sensors(sensors_config):
    ...
    patience = 10
    if EARLY_STOP:    
        early_stopping = EarlyStopping(patience=patience, verbose=True, path='/beegfs/.global1/ws/arsi805e-Test/New/Reduced_Sensors_Models_SA/model_' + str(batch_size) + '_' + str(num_coupling_nodes) + '_' + str(num_subnet_layers +3) + '_' + str(n_epochs) + '_' + str(lr) + '_multiobj' + '/model_checkpoint.pth')
    ...
    torch.save(state, '/beegfs/.global1/ws/arsi805e-Test/New/Reduced_Sensors_Models_SA/model_' + str(batch_size) + '_' + str(num_coupling_nodes) + '_' + str(num_subnet_layers +3) + '_' + str(n_epochs) + '_' + str(lr) + '_multiobj' + '/model_'  + str(batch_size) + '_'  + str(num_coupling_nodes) + '_' + str(num_subnet_layers+3) + '_' + str(n_epochs) + '_' + str(lr) + '_multiobj' + '.pth')
...
def composite_objective(sensor_config, loss_weight, sensor_weight):
    loss = evaluate_model_with_sensors(sensor_config)
    num_active_sensors = np.sum(sensor_config)
    composite_score = (loss_weight * loss) + (sensor_weight * (1 - num_active_sensors / len(sensor_config)))
    return composite_score

def simulated_annealing(initial_state, max_iterations, initial_temp, cooling_rate, loss_weight, sensor_weight):
    current_state = initial_state
    current_temp = initial_temp
    best_state = current_state
    best_score = composite_objective(current_state, loss_weight, sensor_weight)

    for iteration in range(max_iterations):
        candidate_state = current_state.copy()
        sensor_to_toggle = np.random.randint(0, len(candidate_state))
        candidate_state[sensor_to_toggle] = not candidate_state[sensor_to_toggle]

        candidate_score = composite_objective(candidate_state, loss_weight, sensor_weight)

        if candidate_score < best_score or np.random.rand() < np.exp((best_score - candidate_score) / current_temp):
        ...
        dropped_sens = [index+1 for index, value in enumerate(best_state) if not value]
        print(f"Iteration {iteration}: Sensor Config: {best_state}, \nScore: {best_score}")
        print(f"Dropped Sensors: {dropped_sens}")
        logging.info(f"Iteration {iteration}: Sensor Config: {best_state}, \nScore: {best_score}")
        logging.info(f"Dropped Sensors: {dropped_sens}")
        logging.info(f'Active Sensors: {100-len(dropped_sens)}')
...
cooling_rate = 0.95
max_iterations = 500
loss_weight = 0.4  # Adjust based on the importance of loss minimization
sensor_weight = 0.6  # Adjust based on the importance of reducing the number of sensors

# Initial sensor configuration: Randomly initialize half of the sensors as active
#initial_state = np.random.choice([True, False], size=num_sensors, p=[0.5, 0.5])
initial_state = np.array([True]*100)

# Run Simulated Annealing
optimized_config = simulated_annealing(initial_state, max_iterations, initial_temp, cooling_rate, loss_weight, sensor_weight)

# Evaluate the optimized configuration
optimized_loss = evaluate_model_with_sensors(optimized_config)
optimized_num_sensors = np.sum(optimized_config)

print("Optimized Loss:", optimized_loss)
print("Number of Active Sensors:", optimized_num_sensors)